...
import { getProjectById } from "@/lib/dao/TaskDAOAlt"
...
const getProjectData = async (projectId: string) => {
  ...
  const data = await getProjectById(projectId);
    console.log('---------Read project details from edit projects page');
    console.log(data);
    const task = data;
    ...
    const loadProject = async () => {
      try {
        ...
        setFormData({
          ...
          color: projectData.color_code,
          selectedTeamMembers: ["user2", "user3", "user4", "user5"],
          owner: projectData.owner_username,
        })
      } catch (error) {
