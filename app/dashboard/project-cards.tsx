...
export default function ProjectCards({proj}) {
  ...
  projects[0].id = proj[0].id
  projects[0].name = proj[0].name
  projects[0].role = proj[0].role
  projects[0].url = "/dashboard/project/" + proj[0].id
    ...
    <div className="container mx-auto py-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {projects.map((project) => (
          <Link href={project.url} key={project.id} className="block group">
            <Card className="h-full hover:shadow-md transition-shadow border-2 hover:border-primary/20">
              ...
              <CardFooter className="p-2 pt-2 text-xs text-muted-foreground">{project.updatedAt}</CardFooter>
            </Card>
          </Link>
        ))}
      </div>
